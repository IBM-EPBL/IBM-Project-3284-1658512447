...
def history():
    ...
    sql = "select * from histry where username = ?"
    stmt = ibm_db.prepare(conn, sql)
...
def activity():
    ...
    sql = "select count(*) from histry where username = ?"
    stmt = ibm_db.prepare(conn, sql)
    ibm_db.bind_param(stmt,1,username)
    ibm_db.execute(stmt)
    ...
    return render_template('activity.html',username=username,count=count["1"],date=result["TD"])
